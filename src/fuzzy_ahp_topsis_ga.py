...
def create_fuzzy_comparison_matrix(criteria_count, random_state=None):
    """
    Create a fuzzy pairwise comparison matrix for AHP.
    ...
    --------
    np.ndarray
        Fuzzy comparison matrix (n x n x 3)
    """
    ...
    fuzzy_scale = {
        ...
        2: FuzzyNumber(1, 2, 3),      # Weak importance
        3: FuzzyNumber(2, 3, 4),      # Moderate importance
        4: FuzzyNumber(3, 4, 5),      # Moderate plus
        5: FuzzyNumber(4, 5, 6),      # Strong importance
        6: FuzzyNumber(5, 6, 7),      # Strong plus
        7: FuzzyNumber(6, 7, 8),      # Very strong
        8: FuzzyNumber(7, 8, 9),      # Very strong plus
        9: FuzzyNumber(8, 9, 9)       # Extreme importance
    }
...
def fuzzy_ahp_weights(fuzzy_matrix):
    """
    Calculate weights from fuzzy AHP comparison matrix.
...
def normalize_decision_matrix(decision_matrix, criteria_types):
    """
    Normalize decision matrix for TOPSIS.
...
def calculate_topsis_scores(decision_matrix, weights, criteria_types):
    ...
    --------
    np.ndarray
        TOPSIS scores (closer to 1 is better)
    """
